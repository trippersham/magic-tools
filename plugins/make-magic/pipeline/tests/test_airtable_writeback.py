...
def _resolution(
    ...
    return wb.CardResolution(
        record_id=record_id, oracle_id=oracle_id, buckets=buckets, otags=otags
    )
...
class SpyClient:
    ...
    def write_methods(self) -> list[str]:
        return [
            ...
            if m.split(":", 1)[0] in {"POST", "PATCH", "PUT", "DELETE"}
        ]
...
def test_resolve_cards_joins_by_oracle_id() -> None:
    resolved, skipped = wb.resolve_cards(
        _CARDS, name_to_oracle_id=_NAME_TO_OID, card_otag=_CARD_OTAG
    )
...
def test_card_with_unresolvable_oracle_id_is_skipped() -> None:
    ...
    resolved, skipped = wb.resolve_cards(
        cards, name_to_oracle_id=_NAME_TO_OID, card_otag=_CARD_OTAG
    )
    assert resolved == []
...
def test_card_with_no_otag_data_is_skipped() -> None:
    ...
    cards = {
        "recUntagged": "Some Untagged Card"
    }  # -> _OID_UNTAGGED, absent in card_otag
    resolved, skipped = wb.resolve_cards(
        cards, name_to_oracle_id=_NAME_TO_OID, card_otag=_CARD_OTAG
    )
    assert resolved == []
...
def test_plan_writes_is_idempotent_and_keyed_by_record_id() -> None:
    ...
    assert [(w.record_id, w.fields) for w in p1] == [
        (w.record_id, w.fields) for w in p2
    ]
...
def test_wire_guard_rejects_derived_ids_mixed_with_human_id() -> None:
    ...
    client = _client_with_field_map()
    with pytest.raises((wb.HumanFieldWriteError, wb.NonAllowlistFieldError)):
        client._guarded_body(
            [{"id": "recX", "fields": {"fldAAA": ["ramp"], "fldHUMAN": "clobber"}}]
        )
