...
def test_resolve_cached_jar_mode(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """No reactor env, but a fetched shaded jar cached under <data_dir>/xmage/ → the
    committed harness jar is prepended so its fresh XMageBatch shadows the STALE shaded
    class in the dist jar (the ALL-USERS production path — same shadow guarantee as the
    override/reactor branches). cwd stays <data_dir>/xmage/ where CardScanner builds db/.

    Regression guard for the lethality-invariant hole: the cached-dist path used to set
    classpath = str(jar) (harness jar omitted), so the dist jar's stale
    `killed = ... || hasLost()` scoring won class-loading and credited macro-fires as
    kills with the opponent alive — even though the committed harness fix was present."""
    monkeypatch.delenv('MAKE_MAGIC_XMAGE_HOME', raising=False)
...
def test_cached_and_override_build_identical_classpath(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    — the override path prepended the harness jar and the cached path did not."""
    monkeypatch.delenv('MAKE_MAGIC_XMAGE_HOME', raising=False)
...
def test_resolve_tolerates_cached_jar_when_sha_unpinned(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(xr, 'XMAGE_DIST_SHA256', None)
    assert xr.resolve(data_dir=tmp_path).classpath.split(os.pathsep)[-1] == str(jar)
...
def test_ensure_fetches_on_miss(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert install.classpath.split(os.pathsep)[-1] == str(tmp_path / 'xmage' / xr._DIST_JAR_NAME)
...
def test_ensure_stages_then_atomically_publishes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert install.classpath.split(os.pathsep)[-1] == str(final)
