...
def test_crosswalk_slugs_exist_in_dag_snapshot() -> None:
    ...
    snapshot = (
        Path(otag_rollup.__file__).resolve().parents[2]
        / "data"
        / "snapshots"
        / "oracle_tags.json.gz"
    )
...
def test_normalize_drops_template_only_variants() -> None:
    combos = combo_detect.normalize_variants(
        [_VARIANT_CONCRETE, _VARIANT_TEMPLATE_ONLY]
    )
...
def test_factsheet_for_validates_against_contract() -> None:
    fs = deck_factsheet.factsheet_for(
        _SAMPLE_DECK, card_otag=_SAMPLE_OTAG, deck="Sample"
    )
...
def test_factsheet_for_no_focus_is_identical_to_pre_change() -> None:
    ...
    with_none = deck_factsheet.factsheet_for(
        _FOCUS_DECK, card_otag=_FOCUS_OTAG, focus=None
    )
    with_empty = deck_factsheet.factsheet_for(
        _FOCUS_DECK, card_otag=_FOCUS_OTAG, focus=[]
    )
...
def test_factsheet_for_focus_bucket_level_coverage_and_thin() -> None:
    ...
    assert (
        "counters" not in fr["thin_focus"] or fr["coverage_of_focus"]["counters"] >= 1
    )
...
def test_factsheet_for_focus_slug_level_entry_resolves() -> None:
    ...
    fs = deck_factsheet.factsheet_for(
        _FOCUS_DECK, card_otag=_FOCUS_OTAG, focus=["land-ramp"]
    )
...
def test_factsheet_for_off_focus_lists_prominent_non_focus_buckets() -> None:
    ...
    fs = deck_factsheet.factsheet_for(
        _FOCUS_DECK, card_otag=_FOCUS_OTAG, focus=["counters"]
    )
