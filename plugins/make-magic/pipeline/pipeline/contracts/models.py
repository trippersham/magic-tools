...
class Card(BaseModel):
    ...
    oracle_id: str = Field(
        description="Scryfall oracle_id (durable, printing-independent join key)."
    )
    mana_value: float = Field(
        description="Converted mana cost / mana value (Scryfall `cmc`). Lands = 0."
    )
...
class FactSheetShape(BaseModel):
    ...
    cmc_histogram: dict[str, int] = Field(
        description="CMC bucket -> count over nonland cards; buckets '0'..'6','7+'."
    )
...
class FactSheetMana(BaseModel):
    ...
    ramp_sources: int = Field(
        description="Nonland cards that ramp (produce mana / fetch land)."
    )
    fixing_sources: int = Field(description="Ramp sources producing >1 distinct color.")
    pip_counts: dict[str, int] = Field(
        description="Symbol -> pip count across nonland mana costs; keys W/U/B/R/G/C."
    )
...
class FactSheetInteraction(BaseModel):
    ...
    protection: int = Field(
        description="Hexproof/indestructible/ward/shroud/protection/phase-out."
    )
...
class FactSheetStructural(BaseModel):
    ...
    etb_creatures: int = Field(
        description="Creatures with a 'when(ever) ~ enters' trigger."
    )
...
class FactSheetCoverage(BaseModel):
    ...
    categorized_pct: float = Field(
        description="% of nonland cards matched to a census category."
    )
    uncategorized_pct: float = Field(
        description="% of nonland cards left uncategorized."
    )
...
class FactSheetCard(BaseModel):
    ...
    produced_mana: list[str] | None = Field(
        default=None, description="Scryfall produced_mana (may be null)."
    )
...
class FactSheet(BaseModel):
    ...
    keywords: dict[str, int] = Field(
        default_factory=dict, description="Scryfall keyword census (nonzero only)."
    )
    ...
    coverage: FactSheetCoverage = Field(
        description="Categorized vs uncategorized coverage."
    )
    cards: list[FactSheetCard] = Field(
        default_factory=list, description="Raw per-card facts."
    )
...
class InventoryRow(BaseModel):
    ...
    condition: list[str] = Field(
        default_factory=list, description="Condition (multipleSelects)."
    )
    sets: list[str] = Field(default_factory=list, description="Sets (multipleSelects).")
    sources: list[str] = Field(
        default_factory=list, description="Sources (multipleSelects)."
    )
    card_type: str | None = Field(
        default=None, description="Card Type (Scryfall type_line)."
    )
    mana_cost: str | None = Field(
        default=None, description="Mana Cost, e.g. '{2}{W}{U}'."
    )
    cmc: float | None = Field(
        default=None, description="CMC (Scryfall cmc). Lands = 0."
    )
    power_toughness: str | None = Field(
        default=None, description="Power / Toughness, e.g. '2/4'."
    )
    oracle_text: str | None = Field(
        default=None, description="Oracle Text (multilineText)."
    )
    color_identity: list[str] = Field(
        default_factory=list,
        description="Color Identity (multipleSelects): W/U/B/R/G/Colorless.",
    )
    price_tcgplayer: float | None = Field(
        default=None, description="Price (TCGPlayer) currency; Scryfall prices.usd."
    )
    scryfall_url: str | None = Field(default=None, description="Scryfall URL.")
    airtable_record_id: str | None = Field(
        default=None, description="Airtable record id (rec…), a durable join key."
    )
...
class TradeRow(BaseModel):
    ...
    from_source: str = Field(
        description="From (Source) category: Library / Deck / Store / Person."
    )
    to_destination: str = Field(
        description="To (Destination) category: Library / Deck / Store / Person."
    )
    from_deck: str | None = Field(
        default=None, description="From (Deck) — specificity when Source = 'Deck'."
    )
    to_deck: str | None = Field(
        default=None, description="To (Deck) — specificity when Destination = 'Deck'."
    )
    cards_in: list[str] = Field(
        default_factory=list, description="Cards into Destination (card names)."
    )
    cards_out: list[str] = Field(
        default_factory=list, description="Cards out of Destination (card names)."
    )
    status: str | None = Field(
        default=None, description="Status: Draft / Planned / Completed."
    )
    completed_date: str | None = Field(
        default=None, description="Completed Date (ISO date)."
    )
