...
def _stream_cards(
    client: httpx.Client, download_uri: str, max_cards: int | None
...
def run(
    *,
    ...
    try:
        ...
        if (
            not force
            and not is_newer(prior, updated_at)
            and store.table_exists("raw", SOURCE)
        ):
            log.info(
                "oracle_cards: %s not newer than %s; skipping land.", updated_at, prior
            )
