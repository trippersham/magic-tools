...
set" curation closes the residual: folding in the leaf families lifts otag
coverage well above what roots alone reach.
...
Flagged gaps (the "no clean root" set — curated heuristics, not roots):
...
BUCKET_ROOTS: dict[str, frozenset[str]] = {
    ...
    # Burn / reach — damage or life-loss (see module docstring).
...
#: reporting flags these as lower-confidence (no clean roots for
