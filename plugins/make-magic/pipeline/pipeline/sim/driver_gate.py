...
against the real XMage dist jar:
  ...
  2. **Behavioral gate** (:func:`gate_driver`) over the Phase-1 SOLO own-turn goldfish clock,
     ...
     * **Proactive (has a macro):** PASS iff it registers **and** the standard
       ``DRIVER_MACRO_FIRED`` slot-exercise marker appears >=1x across the batch (the macro
       demonstrably executed) **and** the driven own-turn clock is **never-slower** than a
       throwaway pure-CP7 baseline (``medianKillsOwn`` <= baseline + tolerance).
...
class GateResult:
    ...
    check (empty on pass). ``registered`` / ``macro_fired`` / ``steer_fired`` record which
    standard slot-exercise markers were seen in the driven output; ``markers_seen`` is the set
    of those marker names for a loop to introspect. ``driven_median`` / ``baseline_median`` are
    the raw ``medianKillsOwn`` values compared (``-1.0`` = the harness "never killed" sentinel).
    """
...
def gate_driver(
    ...
    **reactive**. ``deck_ref`` is the ``(name, forge_dck_text)`` tuple the solo ``goldfish``
    ...
    over ``games`` games each, then:
      ...
      * PROACTIVE also requires ``DRIVER_MACRO_FIRED`` >=1x AND the never-slower own-turn check;
    ...
    if is_proactive and not macro_fired:
        return _fail(
            f'proactive quad never emitted {DRIVER_MACRO_FIRED_MARKER} over {games} solo games '
            '(the macro slot did not fire — the deterministic win never executed)'
        )
    if not never_slower:
