    ...
    public static CardCollectionView applyContinuousAbility(final StaticAbility stAb, final CardCollectionView affectedCards, final StaticAbilityLayer layer) {
        ...
        for (Card affectedCard : affectedCards) {
            ...
            if (controllerMayPlay && (mayPlayLimit == null || stAb.getMayPlayTurn() < mayPlayLimit)) {
                ...
                // PATCH(sim-ai): getDefinedPlayers can be empty inside a GameCopier'd
                // simulation game (impulse-draw MayPlay effects lose their defined player),
                // which crashed the AI lookahead with IndexOutOfBoundsException. Skip the
                // grant in that case instead of crashing; unreachable in a real game.
                Player mayPlayController = controller;
                if (params.containsKey("MayPlayPlayer")) {
                    final forge.game.player.PlayerCollection mppDefined =
                        AbilityUtils.getDefinedPlayers(affectedCard, params.get("MayPlayPlayer"), stAb);
                    if (mppDefined.isEmpty()) {
                        continue;
                    }
                    mayPlayController = mppDefined.get(0);
                }
