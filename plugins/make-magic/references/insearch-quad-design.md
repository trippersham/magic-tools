...
4. **Φ-mode (rule 4).** **EMPIRICALLY REVISED (nudge sweep — `research/nudge-magnitude-sweep.md`): default every DRIVE deck to a SMALL opportunistic nudge Φ via `seed_nudge_quad(combo, alpha≈2000)`.** A statsig sweep (n=80 solo, n=90 defended over a 6-opponent field) found driver win-rate, solo fire-rate, and goldfish speed **all peak at a small α≈2000** — above both thin (α=0, macro-only) and the heavy dedicated scale (α≈40000). Small-α is never-slower on the goldfish clock (gate-safe); heavy-α over-steers and regresses (the original ladder failure). **So: small nudge = default; thin (α=0) = floor when a nudge can't be expressed; heavy dedicated-scale Φ = AVOID (not a validated superior mode).** *Superseded binary (kept for context):* DEDICATED (full staging Φ) iff commander is a piece OR ≥3 tutors OR Strategy names the combo primary; else COMBO-CAPABLE (thin Φ=0 + macro+P+S). *(Proactive/reactive intuitions were an input to that choice, never the top-level router.)*
